/**
 * @file Contains tests for Quench for testing macro creation
 */
import { createOseMacro } from "../macros";

export const key = "ose.macro.create";
export const options = { displayName: "Macro: Create" };
...
export default ({ describe, it, expect, afterEach }) => {
  afterEach(() => cleanUpMacros());

  describe("Macro", () => {
    it("Can create macro", async () => {
      const macro = await createMockMacro();
      expect(game.macros.contents.find((m) => m.uuid === macro.uuid)).not.equal(
        undefined
      );
    });
