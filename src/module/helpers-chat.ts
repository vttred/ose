...
const canApply: ContextMenuEntry["condition"] = (li) =>
  canApplyDamage(li) && !!li.querySelector(".dice-roll");
...
function canApplyDamage(html: HTMLElement) {
  ...
  switch (applyDamageOption) {
    case CONFIG.OSE.apply_damage_options.originalTarget: {
      return !!html.querySelector(".chat-target")?.getAttribute("data-id");
    }
 ...
 *
 * @param {object} _ - Unused HTMLElement collection
  ...
  options.push(
    {
      ...
      condition: canApply,
      callback: (li) => applyChatCardDamage(li as HTMLElement, 1),
    },
    {
...
export const addChatMessageButtons = (msg: ChatMessage, html: HTMLElement) => {
  ...
  if (roll && canApplyDamage(html)) {
    ...
    buttonContainer.className = "dice-damage";
    buttonContainer.innerHTML = `<button type="button" data-action="apply-damage"><i class="fas fa-tint"></i></button>`;
    roll.append(buttonContainer);

    buttonContainer
      .querySelector('button[data-action="apply-damage"]')
